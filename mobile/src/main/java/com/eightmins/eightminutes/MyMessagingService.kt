...
class MyMessagingService : Service() {
  ...
  private fun sendNotification(conversationId: Int, message: String,
    ...
    val remoteInput = RemoteInput.Builder(EXTRA_VOICE_REPLY).setLabel("Reply by voice").build()
    ...
    val unreadConvBuilder = UnreadConversation.Builder(participant).setLatestTimestamp(timestamp)
        .setReadPendingIntent(readPendingIntent).setReplyAction(replyIntent, remoteInput)

    val builder = NotificationCompat.Builder(applicationContext)
        //        .setSmallIcon(R.drawable.notification_icon)
        //        .setLargeIcon(personBitmap)
        .setContentText(message).setWhen(timestamp).setContentTitle(participant).setContentIntent(readPendingIntent)
        .extend(CarExtender().setUnreadConversation(unreadConvBuilder.build()))

    mNotificationManager!!.notify(conversationId, builder.build())
  }

  private fun createIntent(conversationId: Int, action: String): Intent {
    return Intent().addFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES).setAction(action).putExtra(CONVERSATION_ID, conversationId)
  }
  ...
  companion object {
    val READ_ACTION = "com.eightmins.eightminutes.ACTION_MESSAGE_READ"
    val REPLY_ACTION = "com.eightmins.eightminutes.ACTION_MESSAGE_REPLY"
