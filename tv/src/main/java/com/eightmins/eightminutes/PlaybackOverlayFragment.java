...
public class PlaybackOverlayFragment extends android.support.v17.leanback.app.PlaybackOverlayFragment {
  ...
  private void addPlaybackControlsRow() {
    ...
    mSkipNextAction = new SkipNextAction(sContext);
    mSkipPreviousAction = new SkipPreviousAction(sContext);
    mFastForwardAction = new FastForwardAction(sContext);
    mRewindAction = new RewindAction(sContext);
    ...
    if (PRIMARY_CONTROLS > 3) {
      mPrimaryActionsAdapter.add(new RewindAction(sContext));
    }
    mPrimaryActionsAdapter.add(mPlayPauseAction);
    if (PRIMARY_CONTROLS > 3) {
      mPrimaryActionsAdapter.add(new FastForwardAction(sContext));
    }
